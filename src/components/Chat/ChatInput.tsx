...
export function ChatInput({
  ...
  // 鏅鸿兘瀹氫綅寤鸿妗?
  // 缓存建议框位置，仅在触发显示时计算一次
  const cachedPositionRef = useRef({ top: 0, left: 0 });
  ...
  const calculateSuggestionPosition = useCallback(() => {
    ...
    cachedPositionRef.current = { top: pos.top, left: pos.left };
    return pos;
  }, []);

  // 窗口 resize 时更新可见建议框位置
  useEffect(() => {
    const onResize = () => {
      if (!showCommandSuggestions && !showWorkspaceSuggestions && !showFileSuggestions && !showGitSuggestions) return;
      const pos = calculateSuggestionPosition();
      const xy = { top: pos.top, left: pos.left };
      if (showCommandSuggestions) setCommandPosition(xy);
      if (showWorkspaceSuggestions) setWorkspacePosition(xy);
      if (showFileSuggestions) setFilePosition(xy);
      if (showGitSuggestions) setGitPosition(xy);
    };
    window.addEventListener('resize', onResize);
    return () => window.removeEventListener('resize', onResize);
  }, [showCommandSuggestions, showWorkspaceSuggestions, showFileSuggestions, showGitSuggestions, calculateSuggestionPosition]);

  // 妫€娴嬭Е鍙戠
  // 解析输入触发器（debounce 300ms，避免每次按键都跑5个正则）
  const parseTriggerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const parseTriggers = useCallback(async (newValue: string) => {
    const textarea = textareaRef.current;
  ...
  // 输入变化处理：立即更新 value（保证输入响应），触发器解析 debounce 300ms
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    const newValue = e.target.value;
    setValue(newValue);

    if (parseTriggerRef.current) clearTimeout(parseTriggerRef.current);
    parseTriggerRef.current = setTimeout(() => {
      void parseTriggers(newValue);
    }, 300);
  }, [parseTriggers]);
  ...
  const handleFileDrop = useCallback((files: FileList) => {
    const chips: ContextChipWithId[] = [];
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const filePath = (file as File & { path?: string }).path || file.name;
      chips.push(addChipId({
        type: 'file',
        path: filePath,
        size: file.size,
      }));
    ...
    if (chips.length > 0) {
      setContextChips(prev => [...prev, ...chips]);
    }
  }, []);
