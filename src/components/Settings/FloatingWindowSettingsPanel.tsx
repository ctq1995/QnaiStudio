...
function DotIndicator({ selected }: { selected: boolean }) {
  return (
    <div
      className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${selected ? 'border-primary' : 'border-border'}`}
    >
...
export function FloatingWindowSettingsPanel(props: FloatingWindowSettingsPanelProps) {
  ...
  const savedFloatingWindow = savedConfig.floatingWindow;

  const enabledDirty = savedFloatingWindow.enabled !== floatingWindow.enabled;
  const modeDirty = savedFloatingWindow.mode !== floatingWindow.mode;
  const expandDirty = savedFloatingWindow.expandOnHover !== floatingWindow.expandOnHover;
  const delayDirty = savedFloatingWindow.collapseDelay !== floatingWindow.collapseDelay;
